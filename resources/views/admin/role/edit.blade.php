        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header with-border">
                    <h3 class="card-title text-center">Update Roles Info</h3>
                    <div class="float-right"><a href="{!! url('admin/role') !!}" class="btn-sm bg-gradient-info"><i class="fas fa-arrow-left"></i>&nbsp;Back</a></div>
                </div>
                @if($edit)
                <div class="card-body">
                    {!! Form::open(array('action' => array('Admin\RoleController@update', $user->id), 'method' => 'PATCH')) !!}
                    <!-- Role Name -->
                    <div class="form-group is-warning">
                        <label for="name">Role Name</label>
                        <div class="controls">
                            {!! Form::text('name', $user->name, array('required', 'class'=>'form-control', 'id' => 'name','maxlength'=>'100', 'placeholder'=>'Role Name', 'value'=>old('name'))) !!}
                        </div>
                    </div>
                    <br>
                    <!-- Permissions -->
                            ...
                            <div class="row" style="padding-left: 15px; padding-right: 10px;">
                                @foreach($permissions as $permission)
                                    <div class="col-lg-3 col-md-3 col-sm-6 col-xs-6">
                                        <input type="checkbox" name="permission[]" id="{{ $permission->id }}" value="{{ $permission->id }}" {{ $user->hasPermission($permission->Name) ? 'checked' : '' }} /> {!! $permission->Name !!}
                                    </div>
                                @endforeach
                            </div>
                        </div>
                            ...
                            <div class="row" style="padding-left: 15px; padding-right: 15px;">
                                ...
                                <div class="col-lg-12" style="border: gray dashed 1px; padding: 5px; margin-top: 5px;">
                                    <div class="row" style="margin-left: 2px; color: green; font-weight: 600;">
                                        <input type="checkbox" name="module[]" id="m{{ $module->id }}" value="{{ $module->id }}" {{ $user->hasModule($module->Name) ? 'checked' : '' }} />&nbsp;{!! $module->Name !!}
                                    </div>
                                    ...
                                    @foreach ($allmenus2 as $allmenu)
                                        <div class="row" style="padding-left: 30px; color: darkblue; margin-bottom: 5px;">
                                            <div class="col-lg-12 ml-auto" style="border: gray dashed 1px; padding: 5px; margin-top: 5px;">
                                                &rAarr;&nbsp;<input type="checkbox" name="menu[]" id="mm{{ $allmenu->id }}" value="{{ $allmenu->id }}" class="mainmenu{{ $module->id }}" {{ $user->hasMenu($allmenu->URL) ? 'checked' : '' }} />&nbsp;{!! $allmenu->Name !!}
                                                ...
                                                @foreach ($childmenus2 as $childmenu)
                                                    <div class="col-lg-4 ml-auto">
                                                        <input type="checkbox" name="childmenu[]" id="cm{{ $childmenu->id }}" value="{{ $childmenu->id }}" class="childmenu{{$allmenu->id}}" {{ $user->hasChildMenu($childmenu->URL) ? 'checked' : '' }} />&nbsp;{!! $childmenu->Name !!}
                                                    </div>
                ...
                <div class="card-footer text-right">
                    {!! Form::submit('Save Role', array('class' => 'btn btn-success btn-file')) !!}
