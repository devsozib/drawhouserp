...
@section('content')
    ...
    <div class="content">
        <div class="row">
                            ...
                            @foreach ($requisitions as $group)
                                <div class="accordion" id="faq">
                                    <div class="" id="faqhead2">
                                        <a href="#"
                                            class="btn btn-header-link {{ $group->first()->date == $requisition->po_date ? '' : 'collapsed' }}"
                                    ...
                                    @foreach ($group as $item)
                                        <div id="faq{{ $item->po_date }}"
                                            class="collapse {{ $item->po_date == $requisition->po_date ? 'show' : '' }}"
            ...
            <div class="col-lg-9">
                <div class="card">
                    <div class="card-header with-border">
                        <h3 class="card-title text-center w-75">Purchase Order of <a href=""
                                class="text-link">00{{ $requisition->id }}-{{ $item->company_name }}</a></h3>
                    </div>
                    <div class="card-body">
                        <table class="table table-striped table-bordered">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-group">
                                            <input type="date" disabled class="form-control" name="po_date"
                                                value="{{ $requisition->po_date }}">
                                        </div>
                                    </td>
                                    <td>
                                        <div class="form-group">
                                            <select name="status" id="status" class="form-control" onchange="updatePurchStatus({{ $requisition->id }}, this.value)">
                        ...
                        <div class="card p-4">
                            <div class='item'>
                                <form id="requisitionIssue">
                                    <div id="orderId">
                                        <div class="row">
                                            <input type="hidden" name="purch_id" value="{{ $requisition->id }}">
                                            <div class="col-md-3">
                                                <div class="form-group">
                                            ...
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    <label class="required">Qty Req:</label>
                                                    <input type="text" class="form-control" placeholder="Qty"
                                                        name="ing_qty" required>
                                            ...
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" id="ing_unit_price" name="ing_unit_price"
                                                        class="form-control" placeholder="Unit Price" required><br>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-md-2" style="margin-top: 32px">
                                                <button type="submit" class="btn btn-success addButton"><i
                                        ...
                                        <tbody>
                                            @foreach ($purchItems as $item)
                                                <tr>
                                                    {{-- @php
                                                        $requisitionId = '';
                                                        if (count($item->req_id) > 1) {
                                                            $requisitionId = '0000' . $item->req_id;
                                                        } elseif (count($item->req_id) > 2) {
                                                            $requisitionId = '000' . $item->req_id;
                                                        } elseif (count($item->req_id) > 3) {
                                                            $requisitionId = '00' . $item->req_id;
                                                        } elseif (count($item->req_id) > 4) {
                                                            $requisitionId = '0' . $item->req_id;
                                                        } else {
                                                    ...
                                                    <td>{!! $item->ing_unit_price !!}</td>
                                                    <td>{!! $item->ing_qty !!}</td>
                                                    <td>{!! $item->ing_qty * $item->ing_unit_price !!}</td>
                                                        ...
                                                        <td>
                                                                ...
                                                                <div class="modal-dialog modal-md">
                                                                    <div class="modal-content">
                                                                        ...
                                                                        <div class="modal-body">
                                                                            ...
                                                                            of Purchase number <a href=""
                                                                                class="text-link">{{ $item->purch_id }}</a>
                                                                            ?
                                                                        </div>
                                                                        <div class="modal-footer">
                                                                            <form
                                                                                action="{{ route('purchaseorder.destroy', $item->id) }}"
                                                                                method="post">
                                                                                @csrf
                                                                                @method('delete')
                                                            ...
                                                            <div class="modal fade" id="edit-modal{!! $item->id !!}"
                                                                ...
                                                                <div class="modal-dialog modal-lg ">
                                                                    ...
                                                                    <div class="modal-content">
                                                                        <div class="modal-header">
                                                                            <h4 class="modal-title">Edit Purchase Order item,
                                                                                Name: <a href=""
                                                                        ...
                                                                        <div class="modal-body">
                                                                            <form method="post"
                                                                                action="{{ route('purchaseorder.update', $item->id) }}">
                                                                                @method('patch')
                                                                                @csrf
                                                                                <input type="hidden" name="type"
                                                                                    value="ItemEdit">
                                                                                <input type="hidden" name="purch_id"
                                                                                    value="{{ $item->purch_id }}">
                                                                                    ...
                                                                                    <div class="col-md-3">
                                                                                        <div class="form-group">
                                                                                            <label for="Name">Unit
                                                                                                Price</label>
                                                                                            <input type="number"
                                                                                                value="{{ $item->ing_unit_price }}"
                                                                                                placeholder="ing_qty"
                                                                                                class="form-control"
                                                                                                name="ing_unit_price" required>
                                                                                        </div>
                                                                                    </div>
                                                                                    ...
                                                                                    <div class="col-md-3">
                                                                                        <div class="form-group">
                                                                                            ...
                                                                                            <select name="vendor"
                                                                                                ...
                                                                                                @foreach ($vendors as $vendor)
                                                                                                    <option
                                                                                                        {{ $item->vendor_id == $vendor->id ? 'selected' : '' }}
                                                                                                        value="{{ $vendor->id }}">
                                                                                                        {{ $vendor->name }}
                                                                                                    </option>
                                                                                                @endforeach
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#requisitionIssue').submit(function(event) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 'error') {
                            ...
                            toastr.warning('This item already exists please edit.');
        ...
        function updatePurchStatus(req_id, status){
            $.ajaxSetup({
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        }
